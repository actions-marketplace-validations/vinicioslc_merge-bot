...
class Pull {
  ...
  hasRequiredLabels(labels) {
    if (labels === 'ignore') {
      return true
    } else if (labels.length > 0) {
      console.log('required labels:', labels)
      console.log('loaded labels:', this.labels)
      return labels.every(configLabel => this.labels.includes(configLabel))
    } else {
      return false
    }
  }

  hasBlockingLabels(blockingLabels) {
    return blockingLabels.every(x => !this.labels.includes(x))
  }
   ...
   */
  canMerge(config) {
    console.log('Has required labels:', this.hasRequiredLabels(config.labels))
    console.log('Not have blocking labels:', this.hasBlockingLabels(config.blocking_labels))
    console.log('Is Review Complete:', this.isReviewComplete(config.review_required))
    console.log('Is Checks Complete:', this.isChecksComplete(config.checks_enabled))

    return (
      this.hasRequiredLabels(config.labels) &&
      this.hasBlockingLabels(config.blocking_labels) &&
      this.isReviewComplete(config.review_required) &&
