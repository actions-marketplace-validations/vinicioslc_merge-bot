class Config {
  ...
  static hasRequiredLabels(config, receivedlabels) {
    if (config.labels === 'ignore') {
      ...
      console.log('required labels:', config.labels)
      console.log('loaded labels:', receivedlabels)
  ...
  static hasBlockingLabels(config, labels) {
    return config.blocking_labels.every(x => !labels.includes(x))
  }
}
